...
# 修改1：训练参数 + 在线解码 + 字体显示位置优化
# 修改2：将Matlab在线解码的程序转变成python, 包括训练参数和在线分类
# 修改3：实现频率优选，离线识别率计算+训练参数+在线分类均可自动完成
# 修改4：测试组数可自由选择
# 修改5：在线数据也保存
# Developped by QinZhun on 7.4/2018
...
run_flag = True
symbol = -1  # 全局标志位
channel = [1, 2, 3, 4, 5, 6, 7, 8, 10, 11, 12, 13, 14, 15, 16]
...
class ClassifyThread(threading.Thread):
    ...
    def run(self):
      ...
      #  print("Initializing Complete!")
        ...
        self.readOnlineData = onlineClass.ReadData(channel, timePoint, frequency[freindex], timeNum)
        global feed
        fileName = []
        fileOnline = []
        self.hand = 0
        tt_blooth = serial.Serial('com20', 115200, timeout=0.001)  # 需要修改
        ...
        while run_flag:
            ...
            if not Data_queue.empty():
                ...
                if self.hand == 1 and self.num == 65:
                    self.hand = 0
                    tt_blooth.write('A'.encode())
                ...
                fileOnline[OnlineGroups].write(struct.pack("<h", triggerOnline) + data)
