...
class Board(QFrame):
    ...
    def paintEvent(self, event):
        ...
        if self.sign == -1:
            ...
            self.drawText(painter, 0.75, 0.5, 50, "抓取物体")
            painter.end()
            ...
            self.drawText(painter, 0.5, 0.5, 50, "休息")
            painter.end()
            ...
            painter = QPainter(self)
            painter.begin(self)
            self.drawText(painter, 0.5, 0.5, 50, "本组实验结束-您辛苦啦")
            painter.end()

        elif self.sign == 6:
            painter = QPainter(self)
            painter.begin(self)
            self.drawText(painter, 0.5, 0.5, 50, "请任意选择握拳或者保持休息")
            painter.end()
            ...
            painter.begin(self)
            if feed == 1.5:
                self.drawText(painter, 0.5, 0.2, 50, "您被识别的动作为抓取物体")
            elif feed == 0.5:
                self.drawText(painter, 0.5, 0.2, 50, "您被识别的动作为休息")
            painter.end()
...
class ClassifyThread(threading.Thread):
    ...
    def run(self):
        ...
        self.hand = 0
        global feed
        tt_blooth = serial.Serial('com18',115200,timeout=0.001)  # 需要修改
