...
# Developped by QinZhun on 6.20-7.2/2018
...
class Board(QFrame):
    ...
    # 训练参数
    def param(self):
        train = trainParams.trainParams(channel, timePoint, frequency[freindex], 95, groups)
...
class CaptureThread(threading.Thread):
    ...
    def run(self):
        ...
        offline = offlineParamOptimization.offlineParamOpt(channel, 95, groups)
        print("离线初始化完成")
        ...
        while run_flag:
            ...
            if not Data_queue.empty():
                ...
                if self.formsymbol != self.currentsymbol and self.currentsymbol == 6:
                    ...
                    if self.onlineStarted == 0:
                        self.readOnlineData = onlineClass.ReadData(channel, timePoint, frequency[freindex], 95)
                        self.onlineStarted = 1
