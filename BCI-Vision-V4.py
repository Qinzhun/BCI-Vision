...
class Board(QFrame):
    ...
    # 训练参数
    def param(self):
        print("Initializing Matlab Engine for Training")  # start matlab engine
     #   eng = matlab.engine.start_matlab()
        print("Initializing Complete for Training!")
    ...
    def paintEvent(self, event):
        ...
        if self.sign == -1:
            ...
            painter.begin(self)
            self.drawText(painter, 0.5, 0.5, 30, "实验马上开始-请做好准备-点击按键S开始实验")
            ...
            painter = QPainter(self)
            painter.begin(self)
            self.drawText(painter, 0.5, 0.5, 30, "请任意选择运动或者保持静止")
            painter.end()
            ...
            painter.begin(self)
            if feed == 1.5:
                self.drawText(painter, 0.5, 0.2, 30, "您被识别的动作为左手")
            elif feed == 0.5:
                self.drawText(painter, 0.5, 0.2, 30, "您被识别的动作为右手")
            painter.end()
    ...
    def drawText(self, painter, x, y, size, textString):
        ...
        painter.drawText(x * self.widthWindow-halfLengthText*size*1.2, y * self.heightWindow,
                         textString)
    ...
    def run(self):
        ...
        while run_flag:
            ...
            if not Data_queue.empty():
                ...
                if self.testTrialsContinue and self.num == 95:
